(() => {
  ...
  const ratingHistoryKey = 'monetizabook:rating-toast-names:v1';
  const readRatingHistory = () => {
    try {
      const saved = JSON.parse(localStorage.getItem(ratingHistoryKey) || '[]');
      return Array.isArray(saved) ? saved : [];
    } catch (_) {
      return [];
    }
  };
  const saveRatingHistory = (history) => {
    try { localStorage.setItem(ratingHistoryKey, JSON.stringify(history)); } catch (_) {}
  };
  const randomRatingName = () => {
    const history = readRatingHistory();
    const totalCombinations = ratingFirstNames.length * ratingSurnames.length;
    let candidate = '';
    let attempts = 0;
    do {
      ...
      attempts += 1;
    } while (history.includes(candidate) && attempts < 40 && history.length < totalCombinations);
    const nextHistory = history.length >= totalCombinations ? [candidate] : [...history, candidate];
    saveRatingHistory(nextHistory);
    return candidate;
  };
  ...
  section.innerHTML = `
    <div class="wrap">
      ...
      <div class="testimonial-toast" role="status" aria-live="polite" aria-atomic="true" hidden>
        ...
        <div><strong>NOVA AVALIAÇÃO DE CONTEÚDO RECEBIDA</strong><span class="toast-context">Uma nova avaliação 5 estrelas foi recebida</span><b class="toast-reviewer"><span class="toast-reviewer-name"></span><span aria-label="5 estrelas">★★★★★</span></b><small>aguardando para avaliação</small></div>
  ...
  const showToast = () => {
    toast.querySelector(".toast-reviewer-name").textContent = randomRatingName();
    toast.hidden = false;
